...
- **多维度统计**：出货概率、保底进度、垫抽情况、星级分布、限定 / 常驻区分，条形、卡片、详情、表格四种视图自由切换，卡池隐藏支持两列紧凑选择面板
- **UP 一目了然**：条形视图为当期 UP 五星打标，限定卡池「歪」与「不歪」清晰可辨；卡池卡与汇总卡附「五星出货抽数走势」迷你趋势图
...
- **扁平实色 UI**：干净利落的实色界面，深色 / 浅色主题 + 自定义主色；同一主题下支持 **暖白 / 冷白** 两套色温切换（长时间观看可切到冷白缓解眼疲劳），并可自定义背景图（亮度滑杆 / 显隐 / 玻璃材质可选）
...
| `npm run lint`         | ESLint 静态检查（`src` 目录，严格 0 error / 0 warning）；`lint:fix` 自动修复 |
| `npm run format:check` | Prettier 校验（`src/**/*.{js,css,html}`）；`format` 自动格式化 |
...
- 所有数据（唤取记录、奇藏、等级、设置）仅存储于本机数据库，默认目录为 `%APPDATA%\feibijiubi\FeibiJiubi\`，可在设置页自定义存储位置并一键迁移
...
**Q：如何完全删除数据？**
A：卸载后删除数据目录即可。默认目录为 `%APPDATA%\feibijiubi\FeibiJiubi\`；若你曾在设置页自定义过存储位置，需一并删除对应目录。
