// 背景设置与主题应用（原 src/core/services/settings/background.js → TypeScript 迁移，行为逐行保持一致）
...
// 渲染进程可通过 saveBackgroundSettings 写入的设置项白名单。
// 该通道原本对 key/value 零校验直接落库，且绕开了 main.ts 中 save-setting 的白名单，
// 可被用来覆盖 lastSelectedUid / gameRootDir / accentColor 等任意设置项。
...
// 主色最终会被拼进注入页面的脚本，必须严格限制为 #rgb / #rrggbb，
// 否则可被写成任意 JS 片段形成代码注入。
const HEX_COLOR_RE = /^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/;

// 根据已保存的主色计算需要注入的 :root 主题变量。
// 只返回「变量名 → 变量值」的映射，由调用方以 JSON.stringify 生成字面量，
// 绝不直接拼接成可执行代码片段。
function buildAccentVars(accentColor: string): Record<string, string> {
    if (!accentColor || !HEX_COLOR_RE.test(String(accentColor))) return {};
    ...
    return {
        ...
        '--gradient-primary': accentColor,
        '--gradient-soft': soft,
    };
}

// 把变量映射转成安全的 setProperty 语句：变量名与值都用 JSON.stringify 变成
// JS 字符串字面量，任何引号/分号都只会被当成字符串内容，无法逃逸成代码。
function accentVarsToJs(vars: Record<string, string>): string {
    return Object.keys(vars)
        .map((name) => `document.documentElement.style.setProperty(${JSON.stringify(name)}, ${JSON.stringify(String(vars[name]))});`)
        .join('');
}

// 保存设置到数据库（key 白名单校验 + accentColor 格式校验）
ipcMain.handle('saveBackgroundSettings', async (event: any, key: any, value: any): Promise<{ ok: boolean; error?: string }> => {
    try {
        if (typeof key !== 'string' || !WRITABLE_SETTING_KEYS.has(key)) {
            console.warn(`设置保存被拒绝（不在白名单）: ${String(key)}`);
            return { ok: false, error: 'key not allowed' };
        }
        if (key === 'accentColor' && value != null && value !== '' && !HEX_COLOR_RE.test(String(value))) {
            console.warn('设置保存被拒绝（accentColor 格式非法）');
            return { ok: false, error: 'invalid accentColor' };
        }
        db.prepare(`
...
function brightnessToOpacity(v: any): number {
...
// 改写为异步函数
async function loadBackgroundSettings(): Promise<any> {
    try {
...
// 监听渲染进程请求
ipcMain.handle('loadBackgroundSettings', async (event: any): Promise<any> => {
     // 调用函数获取背景设置
...
// 打开文件选择对话框
ipcMain.handle('selectBackgroundFile', async (event: any, currentPath: any): Promise<any> => {
...
// 加载背景设置
async function loadBackground(mainWindow: any): Promise<void> {
    try {
        // 请求加载背景设置
        const settings = await loadBackgroundSettings();
        const themeMode = settings.themeMode || 'light';
        const colorTemp = settings.colorTemp || 'warm';
        const accentJs = accentVarsToJs(buildAccentVars(settings.accentColor));
...
ipcMain.handle('restoreDefaultBackgroundSettings', async (): Promise<void> => {
    try {
        // 默认背景图片路径
