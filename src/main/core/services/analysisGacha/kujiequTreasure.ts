 ...
 * 原 src/core/services/analysisGacha/kujiequTreasure.js → TypeScript 迁移，行为逐行保持一致。
 *
...
function readLocalOauthCodes(): any[] {
...
function getRegion(playerId: any): any {
...
async function queryRole(playerId: any, oauthCode: any, region: any): Promise<any> {
...
// 将奇藏箱原始映射整理为前端展示结构
function buildTreasureResult(basicBoxes: any, phantomBoxes: any, level: any): any {
    return {
        boxes: {
...
function sortByOauthCode(list: any[]): any[] {
...
const _treasureOauthCache: Record<string, any> = {};
...
function getGameAccountState(gameRootDir: string): any {
  ...
  try {
    const getVal = (k: string): any => {
      try {
        const row: any = db.prepare('SELECT value FROM LocalStorage WHERE key = ?').get(k);
...
async function listTreasureAccounts(playerId: any, isGlobal: any): Promise<any[]> {
    let region: any = null;
...
async function getTreasureBoxes(playerId: any, oauthCode: any, isGlobal: any): Promise<any> {
    // 免 UID：用启动器本地登录态自动定位主账号并拉取（无需游戏内 UID / 抽卡记录）
    if (!playerId) {
        let oc: any = oauthCode;
        let ig: any = isGlobal;
        if (!oc) {
