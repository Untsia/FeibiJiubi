 ...
 * 原 src/core/services/analysisGacha/gameLogReader.js → TypeScript 迁移，行为逐行保持一致。
 *
 ...
 */
function readSharedFile(filePath: string): Buffer | null {
    let fd: any = null;
    try {
 ...
 * 解密后直接以 utf-8 解析为文本。
 */
function decodeXor(buffer: Buffer): string {
 ...
 */
function extractGachaUrl(text: string): string | null {
    if (!text) return null;
 ...
 */
function extractFromBuffer(buffer: Buffer | null): string | null {
 ...
 */
function queryRegistryPath(): Promise<string | null> {
    return new Promise((resolve) => {
        const cmd = 'reg query "HKEY_CURRENT_USER\\Software\\Kuro Game\\Wuthering Waves" /s';
        exec(cmd, { windowsHide: true, timeout: 5000 }, (err: any, stdout: any) => {
 ...
 */
async function locateGameDir(savedPath?: string | null): Promise<string | null> {
    // 1. 用户手动指定
    if (savedPath) {
 ...
 */
async function getGachaUrlFromGameLogs(savedPath?: string | null): Promise<any> {
