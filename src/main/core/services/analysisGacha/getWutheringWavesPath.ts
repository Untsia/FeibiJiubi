// 游戏路径定位与云鸣潮自动抓包（原 src/core/services/analysisGacha/getWutheringWavesPath.js → TypeScript 迁移，行为逐行保持一致）
...
// 执行 REG QUERY 命令
function queryRegistryValue(regPath: string, keyName: string): Promise<string> {
    return new Promise((resolve, reject) => {
        const command = `reg query "${regPath}" /v ${keyName}`;
        exec(command, { encoding: 'binary' }, (err: any, stdout: any, stderr: any) => {
            if (err || stderr) {
...
// 从数据库查询路径并验证其有效性
function queryGamePathFromDb(): Promise<string> {
    try {
        const query = "SELECT path FROM games WHERE path LIKE '%Wuthering Waves.exe%'";
        const row: any = db.prepare(query).get();
        if (row && row.path) {
            ...
            if (!fs.existsSync(logFilePath)) {
                return Promise.reject(`游戏路径无效，日志文件不存在，请检查（路径: ${logFilePath}）`);
            }
            return Promise.resolve(logFilePath);
        } else {
            return Promise.reject('非国服官方启动器需要手动录入游戏库，请检查确保添加的是Wuthering Waves.exe。');
        }
    } catch (err) {
        return Promise.reject(`数据库查询失败: ${err.message}`);
    }
}

// 获取游戏路径（优先数据库，其次注册表）
async function getGamePath(): Promise<string> {
    const errors: any[] = []; // 存储错误信息
...
// 读取日志文件并提取唤取链接
function extractGachaUrl(logFilePath: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
        // 尝试将日志文件复制到临时目录
        fs.copyFile(logFilePath, tempLogFilePath, (copyErr: any) => {
            if (copyErr) {
                console.warn(`[本地日志] 复制失败，自动转向云鸣潮获取...`);
                return openCloudGachaFallback(resolve, reject);
            }
            // 从临时文件读取内容
            fs.readFile(tempLogFilePath, 'utf8', (readErr: any, data: any) => {
                fs.unlink(tempLogFilePath, (unlinkErr: any) => {
 ...
 * 云鸣潮自动化抓包
 */
function openCloudGachaFallback(resolve: (value: any) => void, reject: (reason?: any) => void): void {
    ...
    cloudWin.webContents.on('did-finish-load', () => {
        const row: any = db.prepare("SELECT value FROM settings WHERE key = ?").get('accentColor');
        const __accent = (row && row.value) || '#7c83ff';
        const __rgb = (function(h: string){h=h.replace('#','');if(h.length===3)h=h.split('').map(x=>x+x).join('');const n=parseInt(h,16);return{r:(n>>16)&255,g:(n>>8)&255,b:n&255};})(__accent);
...
document.body.appendChild(toast);
        ...
        `;
        cloudWin.webContents.executeJavaScript(injectNoticeJs).catch((err: any) => console.error('注入提示失败:', err));
    });

    const filter = { urls: ['https://*/*query*'] };
    cloudSession.webRequest.onBeforeRequest(filter, (details: any, callback: any) => {
