// 抽卡分析 IPC 处理（原 src/core/services/analysisGacha/analysisIpc.js → TypeScript 迁移，行为逐行保持一致）
...
// SQL 语句编译缓存：原实现每次 dbGet/dbAll 都重新 prepare，而这里调用的
// SQL 是固定的几条（去重/取最新时间戳等），缓存后避免反复编译。
const stmtCache = new Map<string, any>();
function prepareCached(sql: string): any {
    let stmt = stmtCache.get(sql);
    if (!stmt) {
        stmt = gachaDb.prepare(sql);
...
function dbGet(sql: string, params: any[] = []): Promise<any> {
    try { return Promise.resolve(prepareCached(sql).get(...params)); }
    catch (err) { return Promise.reject(err); }
}
function dbAll(sql: string, params: any[] = []): Promise<any[]> {
    try { return Promise.resolve(prepareCached(sql).all(...params)); }
    catch (err) { return Promise.reject(err); }
}
 ...
 */
ipcMain.handle('get-last-query-uid', async (): Promise<any> => {
    try {
 ...
 */
ipcMain.handle('set-last-query-uid', async (event: any, uid: any): Promise<any> => {
    try {
 ...
 * 获取所有玩家 UID
 */
ipcMain.handle('get-player-uids', async (): Promise<any> => {
    try {
 ...
 */
// 获取唤取记录
ipcMain.handle('get-gacha-records', async (event: any, playerId: any): Promise<any> => {
    try {
 ...
 * 刷新唤取记录
 */
ipcMain.handle('refresh-gacha-records', async (event: any): Promise<any> => {
    try {
        ...
        const poolCount = (poolSummary || '').split('，').filter((s: string) => !s.endsWith('=0')).length;
 ...
 */
ipcMain.handle('import-gacha-from-game', async (event: any): Promise<any> => {
    try {
 ...
 */
ipcMain.handle('detect-wuthering-waves-path', async (): Promise<any> => {
    try {
        // 已保存的手动路径优先
 ...
 */
ipcMain.handle('get-treasure-boxes', async (event: any, playerId: any): Promise<any> => {
    try {
        let pid: any = playerId;
        let oauthCode: any = null;
        let isGlobal: any = null;
...
ipcMain.handle('list-treasure-accounts', async (event: any, playerId: any): Promise<any> => {
    try {
        let pid: any = null, isGlobal: any = null;
        if (playerId && typeof playerId === 'object') {
            pid = playerId.playerId || null;
            isGlobal = playerId.isGlobal != null ? playerId.isGlobal : null;
        } else {
...
ipcMain.handle('get-main-account', async (): Promise<any> => {
    try {
...
ipcMain.handle('get-game-account-state', async (): Promise<any> => {
    try {
        const _gv = getSetting('gameRootDir');
        const gameRootDir = _gv || null;
        let root: string = gameRootDir || '';
        if (!gameRootDir) {
            try {
