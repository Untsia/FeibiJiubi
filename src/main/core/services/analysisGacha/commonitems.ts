// 常驻数据文件管理（原 src/core/services/analysisGacha/commonitems.js → TypeScript 迁移，行为逐行保持一致）
const fs = require('fs');
...
async function ensureCommonItems(): Promise<any> {
...
// 启动初始化：确保数据文件就绪
async function initializeCommonItems(): Promise<void> {
    try {
...
async function loadOrCreateCommonItems(): Promise<any> {
    try {
...
ipcMain.handle('get-common-items', async (event: any, game: any, lang: any): Promise<any> => {
