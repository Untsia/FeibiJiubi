// 控制台中文编码：由 src/core/app/console.js 按系统代码页（ACP）输出，配合 IDE/Windows
// 终端继承系统代码页（中文环境 936=GBK）的默认解码。不再执行 chcp 65001——
// IDE 内置终端直接读子进程字节流，忽略 chcp，且 chcp 与 console.js 的字节输出若不一致
// 反而会制造乱码（UTF-8 字节被 GBK 解 → 锟斤拷）。
const { app, BrowserWindow, Tray, Menu, ipcMain, dialog, shell, screen, protocol } = require('electron');
...
// Electron 代码目录解析：
//   - dev（npm start）：项目根/package.json 的 main=src/main.js，app.getAppPath() = 项目根
//       → appCodeDir() = <appPath>/src
//   - insecure 构建（明文 main.js）：main=.build/src/main.js，app.getAppPath() = asar 根
//       → appCodeDir() = <appPath>/.build/src
//   - 完整构建（bytenode）：明文 loader 写 .build/src/main.js，引导 require('./main.jsc')
//       真实业务代码位于 asar/.build/src/main.jsc，appCodeDir() 仍然锚定到 asar/.build/src
//
// 之前的错误假设：app.getAppPath() 会返回 main.js 所在目录。
// 实测 Electron/electron-builder 下：getAppPath() 返回的是「package.json 所在目录」
// （= asar 根，或项目根）。因此必须把 package.json main 的 dirname 拼上去，
// 才能拿到真正存放 main/preload/assets/renderer 的代码目录，
...
//
// bytenode 下 __dirname 被固化为构建机本地路径，也不能再依赖它。
function appCodeDir() {
  try {
    const base = path.resolve(app.getAppPath());
    let pkg = null;
    try {
      const raw = require('fs').readFileSync(path.join(base, 'package.json'), 'utf8');
      pkg = JSON.parse(raw);
    } catch (_) { pkg = null; }
...
const ASSETS_DIR = () => path.join(appCodeDir(), 'assets');
const RENDERER_DIR = () => path.join(appCodeDir(), 'renderer');
// 版本号 package.json 读取：代码目录(上一级) = 项目根 / 安装包 asar 根，那里有 package.json。
...
// console.warn/error、preload require 异常、bytenode loader 失败堆栈
...
// 这里在 dataFile.js 之前强制统一，双环境共享同一份数据。
try {
...
// 放在 console.js 之后加载，确保失败告警也走 console.js 的系统代码页输出（避免 UTF-8 乱码）。
try {
  require('./feibijiubi_core.node');
} catch (e) {
  console.warn('[native] 核心原生模块未加载（开发模式或构建产物缺失）：', e.message);
}
...
function classifyRuntimeError(context, err) {
...
async function createWindow() {
    ...
    mainWindow = new BrowserWindow({
        ...
        webPreferences: {
            ...
            // 不能用 __dirname：bytenode 编译的 main.jsc 会把 __dirname 固化为构建机路径，
    ...
    // 禁用 HTTP 磁盘缓存：Electron 在 file:// 协议下以「文件路径」作为缓存 key 并忽略
    ...
    const appendProdLog = (tag, line) => {
        const safe = String(tag) + ' ' + String(line).slice(0, 6144);
        try {
            const logPath = path.join(app.getPath('logs'), 'feibijiubi-renderer.log');
            require('fs').appendFileSync(logPath, `[${new Date().toISOString()}] ${safe}\n`);
        } catch (_) {}
    };
    if (!app.isPackaged) {
        const appendDebug = (line) => { try { require('fs').appendFileSync(path.join(app.getPath('logs'), 'renderer-debug.log'), line); } catch (_) {} };
        wc.on('console-message', (event) => { appendDebug(`[r:${event.level}] ${event.message}\n`); });
        wc.on('did-fail-load', (_, code, desc, url) => { appendDebug(`[fail-load] ${code} ${desc} ${url}\n`); });
    } else {
        ...
        wc.on('did-fail-load', (_e, code, desc, url) => appendProdLog('[fail-load]', `c=${code} d=${desc} u=${url}`));
    }
    ...
    wc.on('crashed', (_e, killed) => appendProdLog('[crashed]', killed ? 'killed' : 'crashed'));
    wc.on('render-process-gone', (_e, detail) => appendProdLog('[render-gone]', (detail && detail.reason) || 'unknown'));
    ...
    function buildSnapshotCode(tag) {
...
return JSON.stringify({
  ...
  electronAPI:{hasMin:typeof (window.electronAPI && window.electronAPI.minimizeWindow)==='function',hasMax:typeof (window.electronAPI && window.electronAPI.maximizeWindow)==='function',hasClose:typeof (window.electronAPI && window.electronAPI.closeWindow)==='function',hasDb:typeof (window.electronAPI && window.electronAPI.getSetting)==='function'}
    ...
    function snapshotDom(tag) {
        if (wc.isDestroyed()) return;
        wc.executeJavaScript(buildSnapshotCode(tag)).then(function (json) {
            appendProdLog('[dom-state]', json);
        }).catch(function (e) { appendProdLog('[dom-state-err]', String(e && e.message)); });
    }
    wc.once('dom-ready', () => {
        setTimeout(function () { snapshotDom('t+300ms'); }, 300);
        setTimeout(function () { snapshotDom('t+3500ms'); }, 3500);
    });
    mainWindow.loadURL('local://app/index.html');
...
app.whenReady().then(() => {
    ...
    //   bytenode 编译 main.jsc 时，__dirname 被 V8 固化成构建机 .build/src 的绝对路径，
    ...
    const protocolLog = (line) => {
        try { fsMod.appendFileSync(protocolLogPath, `[${new Date().toISOString()}] ${line}\n`); } catch (_) {}
    };
    // 启动时一次性写 rendererRoot 快照：生产包靠这行定位"为什么 all 404"，
    // 一旦发现记录的是构建机 E:\\... 路径，就可以立即确认是 __dirname 固化漏网。
    protocolLog(`[BOOT] rendererRoot=${rendererRoot} exists=${fsMod.existsSync(rendererRoot) ? '1' : '0'} appCodeDir=${appCodeDir()} packaged=${app.isPackaged ? '1' : '0'}`);
        ...
        protocol.registerFileProtocol('local', (request, callback) => {
            try {
                ...
                if (p.startsWith('/avatars/')) {
                    ...
                    if (!fsMod.existsSync(filePath)) protocolLog(`[404] avatar ${p} -> ${filePath}`);
                ...
                if (p === '/assets/icon.png') {
                    ...
                    if (!fsMod.existsSync(iconPath)) protocolLog(`[404] icon ${iconPath}`);
                ...
                if (p.startsWith('/bg-image/')) {
                    ...
                    if (!fsMod.existsSync(bgPath)) protocolLog(`[404] bg-image ${bgPath}`);
...
// preload.js 暴露 window.electronAPI.reportRenderError，renderer 端
// window.onerror / unhandledrejection 统一走这里。
ipcMain.handle('report-render-error', (_event, payload) => {
    try {
        const fsMod = require('fs');
        const logPath = path.join(app.getPath('logs'), 'feibijiubi-renderer.log');
        ...
        try { fsMod.appendFileSync(logPath, line + '\n'); } catch (_) {}
        // 主进程同步打印一份，开发期 npm start 下直接看终端就能定位。
        console.error(`[render-error][${p.type}] ${p.message} src=${p.source}:${p.lineno}:${p.colno}\n${p.stack || ''}`);
    } catch (_) {}
});

// 版本号统一走顶部的 appCodeDir 版本（详见顶部 appCodeDir 注释 + _readAppVersion 定义）。
// 下面这个重复定义是旧代码残留，避免重复声明直接删掉顶部以下的副本。
// ipcMain 对版本访问仍沿用顶部的 _readAppVersion。
//
// 删除的旧函数定义如下：
// function _readAppVersion() {
//     const candidates = [
...
//     ];
//     for (const p of candidates) { try { ... } catch (e) {} }
