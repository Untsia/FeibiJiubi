...
const avatarDir = path.join(process.env.FEIBIJIUBI_FOLDER_PATH, 'gacha_avatars');
if (!fs.existsSync(avatarDir)) {
    try { fs.mkdirSync(avatarDir, { recursive: true }); } catch (e) {
        console.error('创建头像目录失败:', e.message);
    }
}
...
ipcMain.handle('browse-game-path', async (event, currentPath) => {
