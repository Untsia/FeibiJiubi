/**
 * 趋势线共享工具回归测试（React 侧 utils.ts）
 *
 * 原实现为浏览器端视图脚本 views/shared.js（依赖 document/window），已随 React
 * 迁移移至 src/renderer-react/src/gacha/utils.ts 纯函数。本测试直接 require TS
 * 模块（Node type-stripping），验证与遗留实现一致的抽数语义与可视化输出。
 *
 * 覆盖：
...
const {
  ...
  buildPityTrend,
  buildSparklineSvg,
} = require('../src/renderer-react/src/gacha/utils.ts');
...
test('drawsToNext: 返回到下一个同品质出货的抽数间隔', () => {
  ...
  assert.equal(drawsToNext(recs, recs[0], 5), 3); // index0 → 下一个 5 在 index3
  assert.equal(drawsToNext(recs, recs[3], 5), 3); // index3 → 下一个 5 在 index6
  assert.equal(drawsToNext(recs, recs[6], 5), 1); // 最后一个 5 → 到末尾
  assert.equal(drawsToNext(recs, recs[1], 4), 4); // index1 4星 → 下一个 4 在 index5
  assert.equal(drawsToNext(recs, recs[6], 4), 1); // 末尾无下一个 → length - idx
});
...
test('buildPityTrend: 首尾抽数闭合（起点含本次、终点为上次距本次）', () => {
  ...
  assert.equal(json(buildPityTrend(recs)), jsonArr(2, 3, 1));
});
...
test('buildPityTrend: 最后出五星后有垫抽 → 末尾追加垫抽点', () => {
  ...
  assert.equal(json(buildPityTrend(recs)), jsonArr(1, 3, 1));
});
...
test('buildPityTrend: 有五星但零垫抽 → 不追加 0 抽点', () => {
  ...
  const recs = chrono.slice().reverse();
  assert.equal(json(buildPityTrend(recs)), jsonArr(1, 2));
});
...
test('buildPityTrend: 从未出五星但有垫抽 → 单点为已垫总抽数', () => {
  ...
  const recs = chrono.slice().reverse();
  assert.equal(json(buildPityTrend(recs)), jsonArr(3));
});

test('buildPityTrend: 空数据 → 空序列', () => {
  assert.equal(json(buildPityTrend([])), '[]');
});

// ---------- buildSparklineSvg ----------
test('buildSparklineSvg: 空序列 → 返回空串（不渲染）', () => {
  assert.equal(buildSparklineSvg([]), '');
  assert.equal(buildSparklineSvg(null), '');
});

test('buildSparklineSvg: 单点 → 非空水平线（两端同高）且含该抽数提示', () => {
  const svg = buildSparklineSvg([60]);
...
test('buildSparklineSvg: 单点高度按抽数归一化（抽数越多 y 越小/越高）', () => {
  const low = extractCircles(buildSparklineSvg([10]))[0].cy;
  const high = extractCircles(buildSparklineSvg([80]))[0].cy;
...
test('buildSparklineSvg: 多点 → 每点一个热区且 title 为对应抽数', () => {
  ...
  const svg = buildSparklineSvg(values);
  const circles = extractCircles(svg);
